...
Note that the string derivable from a node $N$, in this case $t_0$, is the concatination of all its leaf nodes. 
...
As MERGEALLVALID only looked at 1 leaf-node or non-terminal on it own (no sequence of leaf-node and/or non-terminals), ARVADA now moves onto bubbling. Where bubbiling now looks at a sequence of leaf-nodes and/or non-terminal. 

\vspace{\baselineskip}
The fundamental next step ARVADA perform is to bubble up a sequecne of sibiling nodes in the current trees $T$ into a new non-terminal.  To bubble up a sequcne $s_1$ in trees $T$ a  new non-terminal is created ${t_s}_1$ with children $s_1$ and all occurrences of $s_1$ in each tree $t$ in $T$ with ${t_s}_1$. Example shown in fig \ref{fig:possible bubble}, where $t_3$ is a bubbled up.
...
Now from the current trees $T$, ARVADA perform GETBUBBLES, collecting all possible bubbles. For each tree $t \in T$, GETBUBBLES collects all proper contiguous subsequences of children in $t$. That is, if a tree contains a node $t_i$ with children $C = c_1, c_2,....c_n$, the potential bubble for this tree include all sequence of C of length greater than 1 and less than n. Note that if the children is a subtree, the structure is maintained, like in fig \ref{fig:possible bubble}. GETBUBBLES returns all these as 1-bubble, and all non-confilicting paris of these as 2-bubbles.
