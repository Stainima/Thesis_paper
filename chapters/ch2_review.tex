...
\node {$t_0$}
        ...
        child {node {n}};
\end{tikzpicture}
\caption{Naive Parse trees after pre-tokenisation in TREEVADA}
\label{fig:pre-tokenised trees}
\end{figure}

Secondly, to tackle the non-deterministic as aspect of ARVADA, by switching to deterministic data-structure for specific opreation. Additionally, also adding two new heuristics bubble length and bubble depth when considering bubble ranks during bubbling.
