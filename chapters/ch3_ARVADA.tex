...
From sections III-D of the original paper \cite{kulkarniLearningHighlyRecursive2021}, string replacement is the check done before before merging any 2 none identical nodes, appendix \ref{fig:Rigorous replacement}. String replacement, given 2 nodes $t_a / c_a$ (replacer) and $t_b / c_b$ (replacee) and a root tree $T_i$, gets all the permutation of the ways $t_a / c_a$ can replace $t_b / c_b$ in $T_i$. Then concatenates each permutation, which becomes a candidate string which is fed into the oracle to validate. For example take,
...
\text{if } L > 1 \;\&\&\; L < 5 \\
\text{if } \text{if} > 1 \;\&\&\; L < 5 \\
\text{if } \text{if} > 1 \;\&\&\; \text{if} < 5 \\
\text{if } L > 1 \;\&\&\; \text{if} < 5
\end{gather*}

From Section~III-D of the original paper~\cite{kulkarniLearningHighlyRecursive2021}, \textit{string replacement} is the verification step performed before merging any two non-identical nodes. Given two nodes, $t_a / c_a$ (the replacer) and $t_b / c_b$ (the replacee), and a root tree $T_i$, the string replacement process generates all permutations of the ways $t_a / c_a$ can replace $t_b / c_b$ within $T_i$. Each permutation is then concatenated to form a candidate string, which is subsequently passed to the oracle for validation.  
...
Let $t_a$ represent $t_1$ and $t_b$ represent $L$.  
After performing all permutations, the candidate strings generated would be:
...
\vspace{\baselineskip}
If the terminal flag is set, the function checks whether the replacee’s character matches the current child’s character. If it matches, the function proceeds; otherwise, it continues to the next iteration of the loop. If the replacee is a non-terminal node, the function instead compares the current child’s \texttt{t\_label} value, proceeding only if it matches the replacee’s \texttt{t\_label}. The replacee is then temporarily replaced by the replacer, and the function recursively calls itself using the same replacer and replacee, but starting at the index of the current iteration.
After the recursive call, the function concatenates the root and passes it to the oracle, which evaluates the candidate string and updates the integer pointer accordingly (the oracle implementation is explained in the next section). Once the recursion returns, the replacee is restored to its original state, and another recursive call is made using the same replacer, replacee, and current index. This ensures that all possible permutations are considered.

\vspace{\baselineskip}
This section is inherently abstract and best understood in conjunction with the source code provided in Appendix~\ref{fig:Rigorous replacement}.

\section{Oracle and other testing}

For $\mathcal{O}$, which was used during string replacement, an ANTLR4-based parser was employed. It was built by following \enquote{Generating a parser in C++ with ANTLR4} by Rahul Gopinath~\cite{gopinathGeneratingParserANTLR42018}. To invoke the parser from C, a wrapper function was implemented (see Appendix~\ref{fig:OracleWrapper}). 

\vspace{\baselineskip}
Given a string, the wrapper uses \texttt{fork()} to create a child process, which is a copy of itself (parent process). In the child process, the ANTLR4 parser is executed with the given string as input. Meanwhile, the parent process (the wrapper) waits for the child process to finish. Once the parser has finished, the parent inspects the child’s return value. If parsing was successful, the wrapper returns \texttt{1}, otherwise it returns \texttt{0}.
